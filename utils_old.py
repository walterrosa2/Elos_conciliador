...
# ==============================================================
# NOVA FUNÇÃO: Verificação de números no histórico dos conciliados
# ==============================================================
def _check_historico(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["valor_abs"] = df["Débito"].abs() + df["Crédito"].abs()
    df["chave"] = df["Conta"].astype(str) + "|" + df["SubPlano Prev"].astype(str) + "|" + df["valor_abs"].astype(str)

    def extrair_numeros(txt):
        return set(re.findall(r"\d+", str(txt)))

    for chave, grupo in df[df["STATUS"] == "Conciliado"].groupby("chave"):
        if len(grupo) == 2:
            idx1, idx2 = grupo.index
            nums1 = extrair_numeros(df.at[idx1, "Historico"])
            nums2 = extrair_numeros(df.at[idx2, "Historico"])
            ...
            df.at[idx1, "Check Histórico"] = txt
            df.at[idx2, "Check Histórico"] = txt

    df.drop(columns=["valor_abs","chave"], inplace=True, errors="ignore")
    return df
...
def salvar_excel(resultado_json: Dict[str, Any], nome_arquivo: str) -> str:
    ...
    if not df_dados.empty:
        df_dados = _check_historico(df_dados)   # <-- APLICA A NOVA LÓGICA
